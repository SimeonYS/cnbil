...
class CnbilPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('cnbil.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `cnbil`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM cnbil WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO cnbil (date, title, link, content)"
